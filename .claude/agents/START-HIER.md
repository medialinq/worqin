...
7. Decomposeer de sprint in concrete taken per agent
   → Gebruik het sprint planning formaat uit .claude/agents/tech-lead.md
...
- Update .claude/agents/TODO.md na elke voltooide taak
...
```
Hervat bij Stap 3 — Time Tracker. Lees .claude/agents/TODO.md voor de huidige status.
```

Een review aanvragen:
```
Activeer de code-reviewer voor alles wat gebouwd is in Stap 2.
Lees .claude/agents/DESIGN.md en .claude/agents/CLAUDE.md voor de reviewcriteria.
```

Naar Fase 2 gaan:
```
Fase 1 is klaar. Plan Fase 2 — Backend.
Lees .claude/agents/TECH.md volledig en .claude/agents/PLAN.md sectie 5 en maak een sprint plan.
```

Security audit aanvragen:
```
Activeer de security-engineer voor een volledige audit.
Lees .claude/agents/TECH.md secties 3, 7, 8 en .claude/agents/security-engineer.md.
```
